...
<a id="add1"></a>
## 10(追記1). メモリの冷却
ボードを横から見ると、メモリとバックプレート間に置かれたTIM(Thermal Interposer Material)が見える。このボードはGDDR6の発熱をこのTIMを介してバックプレートに受けて受動放熱する構造なので、バックプレート側も冷却する必要がある、SoC側へのファン追加だけではメモリに長時間負荷をかける運用時に、ここが足りなくなる。

対応するなら、そのTIMに突き当たる位置にヒートシンク＋強制空冷用的小型ファンを追加してください。筆者は薄型ヒートシンクと追加ファンをバックプレート側に固定して、上向きにエアフローを当てた。

![ボードを側面から撮影した写真](./TIM.jpg)

ここだけ注意点がある。冷却不足になると、tREFI(DRAMのリフレッシュ周期)のリフレッシュが追いつかなくなり、メモリ内容が破損する虞がある。温度が上がるとセルの電荷保持時間が短くなるためで、GDDR6はCPUと共有なので最悪はOSを巻き込んでクラッシュする。そしてこのボードのメモリは温度監視ができない。だから推論中に、こまめに温度を確認してください。メモリ付近が触れないほど熱いなら、エアフローを必ず増やしてください。

## 11(追記2). 消費電力(実測値)
消費電力の実測値は以下ぐらいです。給電端でクランプメーダーを使って測定しました。OS起動時で80W、モデルロード後のidle時で86W、推論中は310Wくらいです。

OS起動直後

![クランプメーターの測定結果写真:OS起動時](./idle-clamp.jpg)

モデルロード後のidle時

![クランプメーターの測定結果写真:モデルロード後・idle時](./loading-clamp.jpg)

推論中

![クランプメーターの測定結果写真:推論中](./thinking-clamp.jpg)

気になるのは、推論中は310Wを常に消費し続けるという点です。20tok/secで16Kトークンの限界まで推論を続けると15分弱くらいの推論になり、かなり暖かくなります。電力はすべて熱になるので、夏は地獄です。

## 12(追記3). 電源の選定
実測値ですが、推論中は交流電源端で310W程度の電力が流れます。スイッチング電源の変換効率を85%と過程すると、264W程度がDC側に出ている計算で、ボードには12V 22Aが流れています。安定のために電源マージンを80%確保する場合、最低でも320W以上の電源を使うのがよいと思います。板は2万円程度ですが、電源は唯一ケチしてはいけない箇所です。
